from auth.schemas import UserInfo, User, UserInDB, UserLogin
...
from auth.utils import verify_token, generate_token
...
async def register(user: User, session: AsyncSession = Depends(get_async_session)):
    if user.password1 == user.password2:
        if not select(users).where(users.c.phone == user.phone).exists:
...
async def login(user: UserLogin, session: AsyncSession = Depends(get_async_session)):
    query = select(users).where(users.c.phone == user.phone)
    ...
    if pwd_context.verify(user.password, user_data.password):
        ...
        return {'success': False, 'message': 'Phone number or password is not correct!'}
